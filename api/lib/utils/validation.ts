...
import type { DropdownOptions } from '../types/Admin';
import dropdowns from '../config/dropdowns.json';

export function validateBeer(beer: BeerFormData, existingBeers: Beer[] = []): BeerValidationResult {
  const errors: BeerValidationError[] = [];
  ...
  if (!beer.slug?.trim()) {
    ...
    const existingSlug = existingBeers.find(b => 
      b.slug === beer.slug && b.uuid !== beer.uuid
    );
    if (existingSlug) {
      errors.push({ 
        field: 'slug', 
  ...
  // Dropdown validation
  const dropdownOptions = dropdowns as DropdownOptions;
  ...
  if (beer.image) {
    ...
    const hasValidExtension = validExtensions.some(ext => 
      beer.image.toLowerCase().endsWith(ext)
    );
...
export function validateUUID(uuid: string, existingBeers: Beer[] = []): BeerValidationResult {
  const errors: BeerValidationError[] = [];
...
export function generateUUID(beerName?: string, style?: string): string {
  // If no name/style provided, just return a UUID
  if (!beerName || !style) {
    return uuidv4();
  }

  const beerSlug = slugify(beerName, { lower: true, strict: true });
  const styleSlug = slugify(style, { lower: true, strict: true });
  
  return `${beerSlug}-${styleSlug}`;
}

export function validateImageUpload(file: any): BeerValidationError[] {
  const errors: BeerValidationError[] = [];
...
export function sanitizeBeerData(beer: any): BeerFormData {
  return {
    name: beer.name?.trim() || '',
